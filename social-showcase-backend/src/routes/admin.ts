...
import { db } from '../db'
import { adminRequired, signToken, type AuthedRequest } from '../middleware/auth'
...
adminRouter.get('/admin/users', adminRequired, async (req: AuthedRequest, res) => {
  ...
  if (q) users = users.filter((u) => u.phone.includes(q) || u.nickname.includes(q) || u.id.includes(q))
...
adminRouter.get('/admin/posts', adminRequired, async (req, res) => {
  ...
  if (q) posts = posts.filter((p) => (p.title || '').includes(q) || (p.text || '').includes(q) || p.id.includes(q))
