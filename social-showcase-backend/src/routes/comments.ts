...
commentsRouter.post('/posts/:postId/comments', authRequired, async (req: AuthedRequest, res) => {
...
commentsRouter.delete('/comments/:id', authRequired, async (req: AuthedRequest, res) => {
  await db.read()
  const c = db.data!.comments.find((x) => x.id === req.params.id)
  if (!c) return res.status(404).json({ message: '评论不存在' })
  if (c.authorId !== req.auth!.userId) return res.status(403).json({ message: '无权限' })
  c.deleted = true
  await db.write()
