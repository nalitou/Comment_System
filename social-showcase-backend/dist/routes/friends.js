...
friendsRouter.post('/friends/request', authRequired, async (req, res) => {
    ...
    const pending = db.data.friendRequests.find((r) => r.status === 'pending' &&
        ((r.fromUserId === meId && r.toUserId === toUserId) || (r.fromUserId === toUserId && r.toUserId === meId)));
    if (pending)
        return res.status(400).json({ message: '已有待处理申请' });
    const t = now();
    const fr = { id: nanoid(), fromUserId: meId, toUserId, status: 'pending', createdAt: t, updatedAt: t };
    db.data.friendRequests.push(fr);
    await db.write();
    return res.json(fr);
});
friendsRouter.get('/friends/requests', authRequired, async (req, res) => {
    await db.read();
    const meId = req.auth.userId;
    const items = db.data.friendRequests.filter((r) => r.toUserId === meId).sort((a, b) => b.createdAt - a.createdAt);
    return res.json(items);
});
friendsRouter.post('/friends/requests/:id/accept', authRequired, async (req, res) => {
    await db.read();
    const meId = req.auth.userId;
    ...
    if (fr.status !== 'pending')
        return res.status(400).json({ message: '状态已变化' });
    fr.status = 'accepted';
    fr.updatedAt = now();
    const f = { id: pairKey(fr.fromUserId, fr.toUserId), userA: fr.fromUserId, userB: fr.toUserId, createdAt: now() };
    db.data.friendships = db.data.friendships.filter((x) => x.id !== f.id);
    db.data.friendships.push(f);
    await db.write();
    return res.json({ success: true });
});
friendsRouter.post('/friends/requests/:id/reject', authRequired, async (req, res) => {
    await db.read();
    const meId = req.auth.userId;
    const fr = db.data.friendRequests.find((r) => r.id === req.params.id);
    if (!fr)
...
friendsRouter.get('/friends', authRequired, async (req, res) => {
    await db.read();
    const meId = req.auth.userId;
    const set = new Set();
...
friendsRouter.delete('/friends/:friendUserId', authRequired, async (req, res) => {
    await db.read();
    const meId = req.auth.userId;
